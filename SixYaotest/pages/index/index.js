...
Page({
   ...
   */
  formSubmit:function(){
    var calyear=this.data.ganzhi;
    var calmonth=this.data.lunarmonth;
    var calday=this.data.lunarday;
    var calhour=this.data.frombirth;
  //计算上卦
  ...
  //计算动爻
    ...
    // console.log("静态上下动爻"+this.data.upperhexagram+" "+this.data.hexagram+" "+this.data.movingline)
    // console.log("动态上下动爻"+calupperhexagram+" "+calhexagram+" "+calmovingline)
    //像跳转页面传送参数
    const tempupperhexagram = this.data.upperhexagram;
    const temphexagram = this.data.hexagram;
    const tempmovingline = this.data.movingline;
    // 当前页面的数据
    wx.navigateTo({
      url:'/pages/show/show',
       // 打开的目标页面
